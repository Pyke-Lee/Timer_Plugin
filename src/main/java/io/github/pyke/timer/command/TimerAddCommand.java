...
public class TimerAddCommand extends CommandAPICommand {
    public TimerAddCommand() {
        super("add");
        withPermission("timer.command.add");
        ...
        executesPlayer((player, args) -> {
            ...
            TimerManager.getInstance().addTime(duration);
            player.sendMessage(Timer.SYSTEM_PREFIX.append(Component.text(String.format("타이머의 시간이 추가되었습니다. (%d시간 %d분 %d초)", h, m, s), NamedTextColor.WHITE)));
        });
    }
}
